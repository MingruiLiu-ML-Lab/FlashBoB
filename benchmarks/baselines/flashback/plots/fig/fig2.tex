...
\begin{tikzpicture}[
    ...
    thick,
    show background rectangle,               % Tells TikZ to draw the
  ...
  \node[Kstyle] (K0)[left=5.75cm of V, yshift=-.6cm] {K};
  \node[Qstyle] (Q0)[below=0.2cm of K0] {Q};
  \node[Vstyle] (V0)[below=0.2cm of Q0] {V};

  \node[whiteNode] (P0)[right=1.3cm of K0, yshift=-0.5cm] {P};
  \node[whiteNode] (O)[right=1.3cm of P0, yshift=-0.5cm] {O};
  ...
  %   \node[whiteNode] (dotdP) [right=2.2cm of Q3, yshift=-0.65cm]
  % {\textdot{dP}};

  \node[titleStyle] (titleleft) [below=0.1cm of P]  {Attention Backward Pass};
  \node[titleStyle] (titleleft) [below=0.1cm of P, xshift=-7.47cm]
  ...
  \draw[->, >=latex, red] (Q2)  -> (dK);
  \draw[->, >=latex, red] (dP)  -> (dK);
  \draw[->, >=latex, red] (dP)  -> (dQ);
  \draw[->, >=latex, red] (K2)  -> (dQ);

  \draw[->, >=latex] (dO)  -> (dV);
  \draw[->, >=latex] (P)  -> (dV);

  \draw[->, >=latex, red] (K0)  -> (P0);
  \draw[->, >=latex, red] (Q0)  -> (P0);
  \draw[->, >=latex] (V0)  -> (O);
  \draw[->, >=latex] (P0)  -> (O);
